...
A question that comes up naturally: what about `std::pmr::vector` with a stack-backed `monotonic_buffer_resource`? That's a "try stack first, fall back to heap" pattern — if you exceed the buffer, the allocator kicks in. `inplace_vector` is different: the capacity is a hard compile-time limit, the storage is guaranteed to live inside the object (which is what enables trivial copyability), and there's no allocator involved at all. If you need a hard capacity bound with no fallback, `inplace_vector` is the right tool. If you want a stack-first optimization with heap as a safety net, `pmr::vector` is still the way to go.

## Constexpr

`std::inplace_vector` is `constexpr`-capable in C++26, including for non-trivial element types — thanks to [P3074R7](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2025/p3074r7.html) and [P3726R2](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2026/p3726r2.html) which resolved the underlying union-lifetime issues. Whether a particular operation can be evaluated at compile time depends on whether the corresponding operations on `T` are valid during constant evaluation.
