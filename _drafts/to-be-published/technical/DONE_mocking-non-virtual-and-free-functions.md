...
We have to create the mocked class the very same way as before except for the `override` specifier and also we don't inherit from any class. Given that we have no `virtual`a, there is nothing to override:
